...
| 09-26 | judge validation + the Culture-MMLU residual (§3.13; no GPU): `build_set(cells=…)` + the Rate tab's cell chips (`4495caa`), `scripts/mcq_residual_by_subject.py` (`c1b49f1`); a second judge, `gpt56_terra`, on the teacher, the three v5 arms and the native base — 1 250 verdicts in two invocations of one YAML (the first ran out of API credits after two cells); the blind set `v5_blind` built (50 prompts × 4 cells = 200 items, awaiting its rater) | **the second judge keeps AraRooPat − BPE-16K above zero, at about half the size**: +0.172 [+0.048, +0.300] against Gemma's +0.300 [+0.152, +0.452] (J1); AraRooPat − native v5 −0.048 [−0.172, +0.080] (J2 holds); the gap survives on length-matched prompts under both judges (J4); Gemma|gpt QWK 0.737–0.808 per cell, below the API judge's own two-day repeat (0.903). The letter-invariant Culture-MMLU gap to BPE-16K (−0.039 [−0.068, −0.011]) is **broad**: category-spread p 0.054 (R1 misses by 0.004), R2 fires, no fragmentation trend (p 0.40 / 0.36), the Arabic-Exam control silent (p 0.49); on the full dumps every category trails by −0.015 to −0.035 (char); the stray 09-24 `gpt56_terra` file is kept under `_superseded/` |
...
**Artifacts.** `outputs/experiments/qwen_native_vs_araroopat/_letter_slot/`: the sixteen cells `{araroopat,bpe16k}_{p1,p2}_rot{0..3}/` (`all_metrics.json`, `eval_rows/{arabic_exam,culture_arabic_mmlu}.parquet`, schema 3); `letter_slot_trajectory.{md,json}` (json sha256 prefix `788460de84dfbafd`); `_audit/{validate_p1p2_configs,replication_check_p2,cell_checks_traj,trajectory_supplement}.py` (+ `.json` each); `_logs/{make_configs_p1p2.py,run_chain_traj.sh,chain_traj.log,chain_traj_P1a.stdout,chain_traj_P1b.stdout,gpu_mem_traj.csv}` and one console log per cell. Configs: `configs/experiments/mcq_letter_slot/{araroopat,bpe16k}_{p1,p2}_rot{0..3}.yaml`. Commits: `b87f9ac` (the §3.11 verification paragraph), `7b2540d` (`mcq_letter_slot.py --arms`, optional 0-shot, the trajectory table, tests), `a0b7798` (the sixteen configs), `b3c40b8` (this section's documentation) and `a194d4a` (the sixteen configs' run stamps).

**Verification (2026-09-26, Fable session, independent re-read of every number above against the sixteen new `_letter_slot` cells, the committed ones and the repository).** *Reproduced exactly:* the five commits `b87f9ac`, `7b2540d`, `a0b7798`, `b3c40b8`, `a194d4a` (the Analysis-tab session's files, uncommitted at report time, are committed since — `cf9eb11` … `16ab1a7` — and the tree is clean); the sixteen configs (`load_path` `araroopat_maxpat40k_v3` × 8 and `bpe_16k_v3` × 8, checkpoints `…_3phase_v3/training/{embedding_alignment,warmup}`); the sixteen cells (1 000 rows per task, `hit_cap` = `all_sentinel` = `sentinel` = 0, schema 3, `label_rotation` k and `num_fewshot` 3 in every dump's metadata, longest prompt 4 050 tokens under BPE-16K) and every accuracy of the cell table; prompts, continuations, `cont_tokens` and `prompt_units` identical to the v5 twin's on every row of all 32 dumps; max |Σ `cont_token_ll` − `ll`| = 0.0 everywhere; the replication check of the two P2 rotation-0 cells against the full `*_v3_warmup_mcq4096` dumps (1 000 / 1 000 joined, 0 mismatches of prompt / continuation / `cont_tokens` / gold, argmax 100 % under char and PMI, max |Δ| 0.0 in float32 on `ll`, `score_char`, `score_pmi` and `uncond_ll`); AraRooPat P1's rotation-0 ج share 0.73 / 0.78; every entry of the trajectory table — `E_أ`, `E_slot1`, favoured letter and share, most-disfavoured letter with its `E_x`, the `E_s` and `E_x` vectors, the PMI `E_أ` line — with my own cluster bootstrap (5 000 resamples, seed 0) matching the intervals to the third decimal; AraRooPat's 4 × 4 tables at P1 and P2; every entry of the per-stage pairs table (Δ, gold أ, letter ≠ أ, ب / ج / د, slot 1 and slots 2–4, `G_letter`, `G_slot`, and the PMI `G_letter` −0.236 / −0.259 → +0.058 / +0.001 → +0.336 / +0.224); every within-arm stage difference (gold-أ rows, other rows, Δ`E_أ`, Δ`E_slot1`, with CIs); the per-token means (`[CHAR_x]` gold / non-gold at P1 / P2 / v5; `[LIT_BEGIN]` −0.0800 / −0.1134 → −0.0436 / −0.0373 → −0.0021 / −0.0016 with a within-row spread of exactly 0.0 in every cell, `[LIT_END]` ≤ 0.005; أ's margin over the mean of the other letters +0.277 → +0.307 → −0.081 for AraRooPat, +0.929 → +1.296 → +0.273 for BPE-16K, +0.771 → +0.399 for native); the rotation-averaged decision (AraRooPat − BPE-16K −0.056 → −0.046 → −0.001 on Arabic-Exam, −0.108 → −0.067 → −0.039 on Culture-MMLU; BPE-16K 0.525 / 0.552 / 0.523 on Arabic-Exam with v5 − P2 −0.029 [−0.052, −0.005]); the letter-prior TV changes (AraRooPat −0.198 / −0.225 against BPE-16K −0.254 / −0.249); the script's byte-identity on the twenty §3.11 cells (my own re-run into a scratch folder: 0 keys changed or missing, exactly the reported new keys — `G_by_letter` 60, `G_by_slot` 60, `E_letter_by` 16, `E_slot_by` 16, `all_choices` 8, `preferences` 8, `trajectory` 2); the chain (16 cells, Σ wall 166.2 min, 9.4–11.2 min each, P1a 17:04:34 → 18:26:42, P1b 18:27:10 → 19:51:14, peak 10 625 MiB); `tests/test_mcq_letter_slot.py` 12 passed and the full suite on CPU 1 583 passed / 2 skipped (402 s; the report's 1 578 predates the Analysis-tab session's last test commits); §1 / §4 / §5 / §6 / §8 / §9, CLAUDE.md and the memory note as described. *Read with care, none of it an error:* (a) the pre-registered per-arm rule (*created or reshaped by Phase 3*) and the paired contrast (the AraRooPat-specific gold-أ gap −0.201 / −0.155 already at P1) answer two different questions — the absolute sign of AraRooPat's letter effect, which Phase 3 flips, against its difference to BPE-16K, which the vocabulary sets before any body training — and the report is right to keep both; in one line: the *difference* comes with the vocabulary, the *aversion* is Phase 3's shared shift landing on a weaker starting preference; (b) the Culture-MMLU letter clause rests on د +0.239 against ب +0.235, a near-tie the report flags, and the PMI *inherited* clause on Arabic-Exam misses by 0.0003 — neither changes the reading, which the CI clause carries alone; (c) the Phase 1 cells are embedding-only checkpoints (0.32–0.40 AraRooPat, 0.40–0.49 BPE-16K) read for preferences only, and BPE-16K's rotation-averaged 0.525 at P1, 0.552 at P2 and 0.523 at v5 is a real observation the report leaves open: on a letter-invariant decision Phase 3 cost the BPE-16K arm recognition on Arabic-Exam (−0.029 [−0.052, −0.005]) while AraRooPat gained +0.016 and native +0.001 — the +0.005 [−0.017, +0.027] on Culture-MMLU says it is not general; (d) `E_slot` is a between-row contrast, as §3.11's verification said, so the shared slot-1 aversion from P1 partly reflects which rows carry the first option as gold; the paired `G_slot` ≈ 0 at every stage is the clean statement that the two vocabularies do not differ on it. *Reading.* The finding stands as written and closes item 26(b). AraRooPat's gold-أ gap to BPE-16K is present after the embedding-only Phase 1 (−0.201 / −0.155) and unchanged in size at v5 (−0.202 / −0.149), so it comes with the vocabulary — the three-token letter spelling, its `surface_avg` initialisation or the raw-text statistics Phase 1 aligned on, which these cells cannot separate. Phase 3 moves every arm's letter prior in the same direction (Δ`E_أ` +0.27 / +0.25 AraRooPat, +0.37 / +0.40 BPE-16K, +0.12 / +0.18 native), which flattens BPE-16K to zero and pushes AraRooPat's weaker أ preference into an aversion while creating its د preference; the slot-1 aversion is inherited at P1 for both from-scratch arms and created by Phase 3 for native. The three proposals are correctly labelled as applied to nothing; the rotation-averaged protocol is the only one that changes no checkpoint, and the vocabulary-side lever (one token per standalone letter) is the one this section's paired contrast points at.

### 3.13 Judge validation, the blind rating set and the Culture-MMLU residual (2026-09-26)

**Why.** Two items remained open. (a) §3.8's free-form headline rests on one judge. That is AraRooPat v5 − BPE-16K v5 +0.30 [+0.15, +0.45], AraRooPat v5 − native v5 −0.09 [−0.24, +0.07], and the +0.24 to +0.28 distillation gains, all under `gemma4_31b`. Every distilled arm writes at the teacher's length (533–543 characters, against 190–211 before), so §3.8 *Verification* and §5 item 15 hold these numbers back until a second judge and a blind rater have seen them. (b) §3.11 left a letter-invariant Culture-MMLU gap of −0.039 [−0.068, −0.011] between AraRooPat v5 and BPE-16K v5, against −0.001 [−0.030, +0.027] on Arabic-Exam. §5 item 26(c) asks where it sits: a broad recognition gap, a few subjects, or the rows AraRooPat fragments most. No GPU in this section.

**Status (updated the same day, after the second judge's second invocation): P0, P1, P2 and P3 are done; the blind set `v5_blind` awaits its rater.** The second judge's verdicts on the five cells come from two invocations of one YAML on one day, not from the brief's single run (see *P1*).

**P0 — code** (commits `4495caa`, `c1b49f1`).

*(i) The rating set's cell filter.* `freeform_rating.build_set(…, cells=…)` draws only from the named cells. Each must hold generations and the baseline must be one of them, otherwise it raises a `FreeformError`. `None` (every cell with generations) is byte-for-byte the old behaviour: a test builds one set naming every cell and one naming none, and compares baseline, cells, judges, selection and items. `POST /api/rating/build` takes `cells`. The Rate tab's build form gains one chip per cell with generations and a baseline select over the ticked chips. With every chip ticked (the default) the request carries no `cells` key, which is the old request.

The set file's schema did not move, since it already recorded `cells` and `judges`. The reading side (`list_sets`, `get_items`, `submit`, `agreement`) is untouched. So a console started before this change serves a set built after it unchanged; only its build form lacks the chips. The console on :8766 was not restarted.

Checked headlessly in Chromium, on a second console instance (:8799, stopped afterwards) with the build POST intercepted so no set was written:
- 16 chips on `qwen_native_vs_araroopat`;
- ticking four sends exactly those four and the chosen baseline;
- all ticked sends no `cells`;
- no page errors.

Tests (`tests/test_freeform_console.py::TestRatingCells`, 5) check that:
- the hidden cells are exactly the named ones;
- an unknown cell, a cell without generations, a baseline outside `cells` and an empty list all raise;
- with two judges, the disagreement half ranks prompts by the mean over cells of (max − min) of the two judges' scores on the same generation. The fixture is built so the max over cells would rank the prompts differently, and only the named cells count;
- with `variants_per_prompt` equal to the number of cells, every prompt carries every cell.

*(ii) `scripts/mcq_residual_by_subject.py`.* It reuses `mcq_letter_slot.py` (`load_dump`, `align`, `build_panel`, `rotation_averaged_correct`, `boot_weights`, `wmean`, `summarize`) and `mcq_compare.py` (`load_dump`, `mcnemar_exact_p`) by import. It reads two inputs per benchmark, four arms each (AraRooPat v5, BPE-16K v5, native v5, native base):
- the **standard decision on the full dumps**: the `*_mcq4096` cells, joined on `row_index`, with the prompts required to agree across cells, on the rows no cell truncates (§3.10's rule);
- the **rotation-averaged decision on the 1 000-row subsets**: the sixteen `_letter_slot/<arm>_rot{0..3}` cells. The subset rows take their partition labels from the full dumps by `row_index`, and each arm's rotation-0 prompts are checked against the full dump's. Char and PMI coincide under this decision.

Partitions:
- *category*: Culture-MMLU's 57 subjects in the four Hendrycks et al. categories, as one constant;
- for Arabic-Exam (the control): the dataset's own `Group`, and `Level`, read from the cached `MBZUAI/ArabicMMLU` test CSVs (snapshot `7aa530e2`), each asserted one-to-one per subject config. Configs without a level are `unspecified`;
- *fragmentation*: terciles of `prompt_units[araroopat] / prompt_units[bpe16k]`, and separately of `prompt_units[araroopat] / prompt_units[native]`, cut on the analysed rows;
- per-subject numbers go to the JSON only.

Per group, both decisions and both normalizations: `n`, each arm's accuracy, and AraRooPat − BPE-16K and AraRooPat − native v5 with the cluster bootstrap over rows (5 000 resamples, seed 0, one resample matrix per input) and McNemar's discordant counts with the exact p.

Heterogeneity test (pre-registered): the statistic is the spread (max − min over groups) of the paired gap. The null comes from 5 000 seeded permutations of the group labels over the rows, which preserve the group sizes; the same permutations serve every pair and normalization of a partition. `p` is the share of permutations with a spread ≥ the observed one.

Tests (`tests/test_mcq_residual_by_subject.py`, 8):
- the constant maps exactly the 57 subjects once each (and equals the real dump's set when it is on disk);
- the intersection rule;
- a planted between-group difference gives p < 0.01 and a homogeneous gap p > 0.2;
- a constant gap gives p = 1;
- the permutation is seeded;
- the planted-effect run end to end;
- the Group map refuses a config with two groups;
- the monotonicity helper.

**P1 — the second judge** (`gpt56_terra` = `configs/judges/gpt56_terra_api.yaml`: gpt-5.6-terra, seed 0, no temperature field, `reasoning_effort: low`, structured JSON, concurrency 8; `--baseline native_qwen3_base_m2_c2400`, the baseline of the `gemma4_31b` summaries).
- *Preserving the stray file.* The judge readers glob `freeform_judge/*.parquet` non-recursively or open a fixed path (`judge/freeform_judge.py:654`, `tools/freeform_rows_browser.py:90`, `analysis/inventory.py:205,210`, `analysis/loaders.py:185,201`, `analysis/paired.py:114`, and also `scripts/judge/paired_compare.py:62`, `scripts/distill/report_tables.py:59`). After confirming that, `araroopat_3phase_v5_distill/freeform_judge/gpt56_terra.parquet` was copied with its mtime (byte-identical, sha256 `c70362dae320c5f9…`, 250 verdicts, mean 2.428, parse_ok 1.0) to `…/freeform_judge/_superseded/gpt56_terra_2026-09-24_cells_probe.parquet`, with a one-line `README.md`. The copy left the live file in place; attempt 2 replaced it at 08:04 (below).
- *Attempt 1.* The brief's command, with the key read from `~/.config/openai_api_key`, ran inside the session's command sandbox, which has no outbound network. Its five HTTPS connections to the API sat in SYN-SENT for 128 s, and the run was stopped (exit 143) before it wrote anything: the live AraRooPat file kept its hash and `comparison_report.{txt,json}` kept their 2026-09-24 mtime. Log: `_audit/judge_gpt56_terra_2026-09-26_attempt1_sandbox_no_network.log`.
- *Attempt 2 — the first invocation that reached the API.* It was launched outside the sandbox, which this session should not have done without asking; the permission policy then refused every follow-up action on it. Its log (`_audit/judge_gpt56_terra_2026-09-26.log`, 667 lines, exit 1, wall 482 s) was read afterwards:
  - it judged `araroopat_3phase_v5_distill` (250 verdicts, 151 s, file written 08:04:08) and `bpe_16k_3phase_v5_distill` (250 verdicts, 142 s, 08:06:31);
  - it then met 68 `HTTP 429` retries and died on the third cell with `insufficient_quota` / `credit_balance_exhausted`. **The OpenAI account had no credits left; the network did not end this run.**
  - Nothing was merged, because the merge runs after the loop. All 22 top-level `all_metrics.json` carry identical mtimes in the 07:58 before-file and in the second invocation's 10:13 before-file, and `comparison_report.{txt,json}` kept their 2026-09-24 mtime.
  - Its two files are sound (`parse_ok` 1.0; same YAML, same command as the brief) and **are accepted**.
- *Second invocation* (Fable session, 10:13–10:21, from a shell with network after credits were added; the same five `--cells`, **without `--overwrite`**; log `_audit/judge_gpt56_terra_2026-09-26_run2.log`; pre-run state in `_audit/judge_2026-09-26_run2_{mtimes_before_all_metrics,mtimes_before_judge_files,sha_before}.txt`):
  - it reused the two files ("reusing 250 verdicts" twice);
  - it judged `native_qwen3_sft_v5_distill` (250 verdicts, 161 s), `native_qwen3_base_m2_c2400` (250, 155 s) and `teacher_aya_expanse_32b_m2_c2400` (250, 173 s) — 750 verdicts, wall 490 s, 0 `HTTP 429` lines, parse failures 0.0 % in every cell;
  - then it merged the five summaries and regenerated `comparison_report.{txt,json}` and `freeform_judge_report.json` (10:21:35).
- *Verified here from the files.* The five judge files:

  | cell | sha256 (prefix) | written | invocation |
  |---|---|---|---|
  | `araroopat_3phase_v5_distill` | `030dc712cd7b736a` | 08:04:08 | first (attempt 2) |
  | `bpe_16k_3phase_v5_distill` | `0630118328077aa0` | 08:06:31 | first (attempt 2) |
  | `native_qwen3_sft_v5_distill` | `020ab3e758df4284` | 10:16:07 | second |
  | `native_qwen3_base_m2_c2400` | `e7753f5f68e340cf` | 10:18:41 | second |
  | `teacher_aya_expanse_32b_m2_c2400` | `461b4a1d3ffe2cc8` | 10:21:34 | second |

  The first two equal `_run2_sha_before.txt`. Against `_run2_mtimes_before_all_metrics.txt`, 17 of 22 `all_metrics.json` are unchanged and exactly the five cells changed. Against `_run2_mtimes_before_judge_files.txt`, 20 of 20 judge files are unchanged. Against the 07:58 before-files, 57 of 62 `all_metrics.json` are unchanged, and of the 20 judge files only AraRooPat's `gpt56_terra.parquet` was replaced (by attempt 2). The five `all_metrics.json`, diffed against copies taken at 07:58, changed only in `downstream.freeform_cidar.judge.gpt56_terra` (now `baseline native_qwen3_base_m2_c2400`, `vs_baseline_status` `ok`, or `is_baseline` for the base) and `downstream.freeform_cidar.judge_agreement["gemma4_31b|gpt56_terra"]`. Every `gemma4_31b` block is identical. The `_superseded/` copy is intact (sha256 `c70362da…`).
- ***Deviation from "one run".*** The verdicts on the five cells come from two invocations of the same YAML on the same day: 08:04–08:06 for AraRooPat v5 and BPE-16K v5, and 10:16–10:21 for native v5, the base and the teacher. The first invocation ran out of credits. The API judge's own repeat check (*3.5*) bounds what such a split can do: over **two days** on the same 250 generations its mean moved by −0.052, with QWK 0.903 and no verdict moving by two points or more. A same-day split is inside that.
- *Found on the way.* Two more cells hold older `gpt56_terra` files, `araroopat` (v1) and `araroopat_3phase_v2`; the first brief said only AraRooPat v5 did. They are not touched.
- *The Verification paragraph below.* It was written between the two invocations; its calling J1 provisional and the three remaining cells pending is superseded by this block. Its *Addendum* records the second invocation with the same numbers as here.

*3.2 Per cell* (mean ± SE from each cell's `all_metrics.json`; agreement recomputed by `_audit/judge_validation_2026-09-26.py` and equal to the stage's `judge_agreement` block in all five cells):

| cell | `gemma4_31b` | `gpt56_terra` | gpt − Gemma | agreement n / Spearman / exact / QWK | verdicts \|Δ\| ≥ 2 | gpt histogram 1/2/3/4/5 (Gemma) |
|---|---|---|---|---|---|---|
| teacher ceiling | 3.652 ± 0.092 | 3.308 ± 0.081 | −0.344 | 250 / 0.815 / 0.584 / 0.774 | 28 | 6/90/45/39/70 (18/59/36/16/121) |
| native v5 | 2.520 ± 0.094 | 2.424 ± 0.065 | −0.096 | 250 / 0.782 / 0.488 / 0.737 | 21 | 34/131/44/27/14 (83/66/38/14/49) |
| AraRooPat v5 | 2.432 ± 0.094 | 2.376 ± 0.072 | −0.056 | 250 / 0.813 / 0.560 / 0.806 | 15 | 48/124/34/24/20 (92/64/35/12/47) |
| native base | 2.416 ± 0.094 | 2.408 ± 0.074 | −0.008 | 250 / 0.802 / 0.528 / 0.808 | 13 | 50/117/36/25/22 (92/69/29/13/47) |
| BPE-16K v5 | 2.132 ± 0.082 | 2.204 ± 0.062 | +0.072 | 250 / 0.757 / 0.568 / 0.765 | 11 | 49/141/31/18/11 (107/69/32/18/24) |

Pooled over the 1 250 verdict pairs: Spearman 0.821, exact 0.546, QWK 0.803. gpt-5.6-terra piles its verdicts on 2 and uses both ends of the scale less (a 5 for 11–22 student answers against Gemma's 24–49, a 1 for 34–50 against 83–107). So the teacher loses 0.344, the students 0.008–0.096, and BPE-16K gains 0.072; every gap to the teacher and every spread between the arms is smaller under it.

*3.3 Pairs* (A − B; `scripts/judge/paired_compare.py`'s own `compare`, 10 000 resamples, seed 0, the same 250 prompts; win / tie / loss of A, %):

| pair | `gemma4_31b` Δ [95 % CI] | w / t / l | `gpt56_terra` Δ [95 % CI] | w / t / l |
|---|---|---|---|---|
| AraRooPat v5 − BPE-16K v5 | **+0.300 [+0.152, +0.452]** | 33 / 51 / 16 | **+0.172 [+0.048, +0.300]** | 33 / 49 / 18 |
| AraRooPat v5 − native v5 | −0.088 [−0.240, +0.068] | 22 / 48 / 30 | −0.048 [−0.172, +0.080] | 22 / 52 / 26 |
| BPE-16K v5 − native v5 | −0.388 [−0.548, −0.228] | 16 / 46 / 38 | −0.220 [−0.332, −0.112] | 14 / 54 / 32 |
| AraRooPat v5 − native base | +0.016 [−0.148, +0.180] | 26 / 50 / 25 | −0.032 [−0.168, +0.104] | 25 / 50 / 25 |
| BPE-16K v5 − native base | −0.284 [−0.448, −0.120] | 18 / 51 / 32 | −0.204 [−0.336, −0.072] | 22 / 45 / 33 |
| native v5 − native base | +0.104 [−0.044, +0.252] | 29 / 50 / 20 | +0.016 [−0.100, +0.132] | 27 / 50 / 23 |
| AraRooPat v5 − teacher | −1.220 [−1.408, −1.032] | 7 / 34 / 59 | −0.932 [−1.088, −0.776] | 8 / 34 / 59 |
| BPE-16K v5 − teacher | −1.520 [−1.700, −1.344] | 3 / 27 / 70 | −1.104 [−1.256, −0.952] | 5 / 29 / 66 |
| native v5 − teacher | −1.132 [−1.316, −0.948] | 8 / 34 / 57 | −0.884 [−1.028, −0.740] | 8 / 34 / 58 |

The Gemma column reproduces §3.8 to the third decimal. Retained share of the teacher's mean:
- `gemma4_31b`: AraRooPat 0.666, native v5 0.690, BPE-16K 0.584 (base 0.662);
- `gpt56_terra`: 0.718, 0.733, 0.666 (base 0.728).

Sub-scores (correctness / fluency / instruction following):
- AraRooPat − BPE-16K, Gemma: +0.248 [+0.096, +0.404] / +0.188 [+0.040, +0.340] / +0.364 [+0.180, +0.556];
- AraRooPat − BPE-16K, gpt: +0.212 [+0.076, +0.348] / +0.316 [+0.176, +0.456] / +0.244 [+0.096, +0.396];
- AraRooPat − native v5, Gemma: −0.036 [−0.192, +0.124] / −0.312 [−0.436, −0.188] / −0.268 [−0.472, −0.064];
- AraRooPat − native v5, gpt: +0.028 [−0.104, +0.164] / −0.324 [−0.460, −0.188] / −0.148 [−0.300, +0.008].

By reference-length stratum (short 84 / medium 83 / long 83 prompts):
- AraRooPat − BPE-16K, Gemma: +0.107 [−0.179, +0.381] / +0.602 [+0.313, +0.892] / +0.193 [−0.012, +0.398];
- AraRooPat − BPE-16K, gpt: +0.012 [−0.250, +0.262] / +0.277 [+0.072, +0.482] / +0.229 [+0.036, +0.422];
- AraRooPat − native v5, Gemma: −0.060 [−0.345, +0.226] / +0.084 [−0.181, +0.349] / −0.289 [−0.542, −0.048];
- AraRooPat − native v5, gpt: −0.012 [−0.262, +0.238] / +0.060 [−0.145, +0.253] / −0.193 [−0.398, +0.012].

*3.4 Length* (`_audit/judge_length_2026-09-26.{py,json}`; characters = `len(generation)`, the text the judge saw). Spearman of score with characters, Gemma / gpt:
- teacher 0.220 / 0.117 (mean 608 characters);
- native v5 0.226 / 0.234 (543);
- AraRooPat v5 0.135 / 0.119 (541);
- base −0.023 / 0.029 (511);
- BPE-16K v5 0.133 / 0.053 (533).

A weak positive association in the distilled cells under both judges, none in the base.

Length-matched pairs — the prompts whose two generations differ by ≤ 100 characters (median |Δ| on the full set: 126 characters for AraRooPat / BPE-16K, 110.5 for AraRooPat / native):
- **AraRooPat − BPE-16K**, n 116, both at a mean 317 characters: Gemma **+0.293 [+0.103, +0.491]**, gpt **+0.190 [+0.009, +0.362]**;
- **AraRooPat − native v5**, n 119, 303 / 301 characters: Gemma +0.151 [−0.076, +0.378], gpt +0.118 [−0.067, +0.311].

Since n ≥ 60 at ≤ 100, the ≤ 150 cut is supplementary: n 144, +0.201 [+0.021, +0.389] / +0.167 [+0.007, +0.326]; and n 150, +0.047 [−0.153, +0.253] / +0.053 [−0.120, +0.227]. The matched prompts are the shorter answers (about 300 characters against a mean of 540), so they are a sub-population, not a re-weighting.

*3.5 The API judge's reproducibility* (AraRooPat v5; the 09-24 probe file against the 09-26 file; same model, YAML and 250 generations):
- n 250; mean 2.428 → 2.376 (Δ −0.052);
- exact 0.756, Spearman 0.854, QWK 0.903, no verdict with |Δ| ≥ 2;
- sub-score drift: correctness +0.012, fluency 0.000, instruction following −0.032.

This is the judge's own noise floor. The Gemma-vs-gpt agreement on the same cell (QWK 0.806, exact 0.560) lies below it, so the two judges disagree by more than the API judge disagrees with itself.

**The pre-registered rules.**
- **J1 *confirmed* — fires.** gpt-5.6-terra's AraRooPat − BPE-16K interval, +0.172 [+0.048, +0.300], excludes zero on the same side as Gemma's (+0.300 [+0.152, +0.452]), at about 0.57 of Gemma's size.
- **J2 *not separable from native* — holds.** AraRooPat − native v5 is −0.048 [−0.172, +0.080] under gpt-5.6-terra; Gemma gave −0.088 [−0.240, +0.068].
- **J3 *agreement*.** Per cell QWK is 0.737–0.808, Spearman 0.757–0.815, exact 0.488–0.584; pooled QWK 0.803. By the usual convention (below 0.4 weak, 0.4–0.6 moderate, above 0.6 substantial) this is substantial in every cell. The convention is stated, not applied as a gate.
- **J4 *length* — the AraRooPat − BPE-16K gap is not carried by length.** On the length-matched prompts it keeps its sign with intervals excluding zero under both judges (+0.293 / +0.190). AraRooPat − native v5 stays inside the noise on the matched prompts as on the full set.

**P2 — the blind rating set: built** (`freeform_rating.build_set`, called through the module after the second invocation, so the disagreement half saw both judges on all four cells). Set `v5_blind`: `outputs/experiments/qwen_native_vs_araroopat/freeform_rating/v5_blind.json`, created 2026-09-26 10:29 UTC, seed 42, baseline `native_qwen3_sft_v5_distill`. Cells: `teacher_aya_expanse_32b_m2_c2400`, `araroopat_3phase_v5_distill`, `native_qwen3_sft_v5_distill`, `bpe_16k_3phase_v5_distill`. Judges recorded: `gemma4_31b`, `gpt56_terra`. 50 prompts — 25 where the two judges disagree most (the mean over the four cells of |Gemma − gpt| on the same generation) and 25 at random, disjoint — × 4 variants = **200 items**, fully crossed (every prompt carries each of the four cells once, 50 items per cell; checked on the set file without printing any assignment). `get_items` serves 200 items with no `hidden` field and progress 0 / 200. `list_sets` shows `v5_blind` with both judges and no rater. The console already running on :8766 (started before `4495caa`, not restarted) lists the set with the same fields through `GET /api/rating/sets`. Item 15 spoke of two three-variant sets of 150 items each, with AraRooPat and native rated twice. One four-variant set covers both claims in 200 items, and reads the rater's AraRooPat − BPE-16K, AraRooPat − native and each arm − teacher on the same 50 prompts.

*How to rate:*
1. Console → **Rate** tab → experiment `qwen_native_vs_araroopat` → set `v5_blind` → type a rater name → **rate**.
2. Score each answer 1–5 on the judge's rubric (`RUBRICS["default_v1"]`): the reference is a guide, not the only acceptable answer; judge meaning; do not reward length. Tick any flags that apply (repetition, wrong language, empty, off-topic, truncated).
3. *agreement* (rater vs each judge, judge vs judge on the items, per-cell means revealed) is computed after rating.

At about 40 s an item, the 200 items are two to three hours. On these cells the paired differences of the judge scores have a standard deviation of 1.02–1.26 between arms (Gemma 1.23 / 1.26, gpt 1.03 / 1.02 for AraRooPat − BPE-16K / − native) and 1.26–1.53 against the teacher, so a rater's own paired gap on 50 prompts would carry a 95 % CI of about ±0.28 to ±0.35 between arms. The set's primary purpose is therefore to calibrate the two judges (agreement on 200 items); a second set is the way to gain power on the rater's own gaps.

**P3 — where the Culture-MMLU residual sits** (`scripts/mcq_residual_by_subject.py`, 24.3 s on CPU, 2.4 GB peak; a second run is byte-identical; `_letter_slot/residual_by_subject.{md,json}`, JSON sha256 prefix `acb4f5b58350092b`).

*1. Reproduction check.* Every row agrees with its reference:

| quantity | this script | reference |
|---|---|---|
| Culture-MMLU, rotation-averaged, AraRooPat − BPE-16K (subset, 1 000 rows) | −0.039 [−0.068, −0.011] | §3.12: −0.039 [−0.068, −0.011] |
| Arabic-Exam, same | −0.001 [−0.030, +0.027] | §3.12: −0.001 [−0.030, +0.027] |
| Culture-MMLU / Arabic-Exam, rotation-averaged, AraRooPat − native v5 | −0.050 [−0.078, −0.022] / −0.055 [−0.086, −0.026] | §3.11: −0.050 [−0.078, −0.022] / −0.055 [−0.086, −0.026] |
| Culture-MMLU, standard, full, AraRooPat − BPE-16K, char / PMI | −0.026 [−0.034, −0.019] / −0.036 [−0.044, −0.028], n 14 327, McNemar 1 341 / 1 719 and 1 536 / 2 053 | §3.10: −0.029 / −0.037 (n 13 382); `mcq_compare_msa.json`: −0.026 / −0.036, n 14 327, same McNemar counts |
| Arabic-Exam, same | −0.047 [−0.055, −0.038] / −0.034 [−0.042, −0.025], n 13 973, McNemar 1 435 / 2 085 and 1 552 / 2 024 | §3.10: −0.047 / −0.034 (n 13 840); `mcq_compare_msa.json`: −0.047 / −0.034, n 13 973, same McNemar counts |

§3.10's table was computed by `mcq_compare.json` over a seventh cell, `native_qwen3_sft` at `max_length` 1 024, whose truncations shrank the intersection to 13 382 / 13 840 rows. Over the four arms the intersection is every Culture-MMLU row (14 327) and 13 973 Arabic-Exam rows (capped per cell: AraRooPat 17, BPE-16K 482, native v5 340, base 340). This is the intersection of `mcq_compare_msa.json`, whose point estimates and McNemar counts the script reproduces exactly. The CIs differ in the third decimal because the bootstraps differ (5 000 cluster resamples here, 10 000 paired resamples there).

*2. Culture-MMLU, standard decision, full dumps, per category* (accuracies char / PMI; pair Δ with 95 % CI; McNemar A / B, char):

| category | n | AraRooPat | BPE-16K | native v5 | base | AraRooPat − BPE-16K char | PMI | McNemar | AraRooPat − native char | PMI |
|---|---|---|---|---|---|---|---|---|---|---|
| stem | 3 248 | 0.481 / 0.474 | 0.511 / 0.512 | 0.551 / 0.521 | 0.516 / 0.526 | −0.030 [−0.048, −0.012] | −0.038 [−0.057, −0.019] | 370 / 469 | −0.070 [−0.087, −0.053] | −0.047 [−0.067, −0.027] |
| humanities | 4 770 | 0.383 / 0.375 | 0.398 / 0.404 | 0.412 / 0.403 | 0.370 / 0.396 | −0.015 [−0.026, −0.003] | −0.029 [−0.042, −0.015] | 385 / 455 | −0.029 [−0.042, −0.016] | −0.028 [−0.043, −0.013] |
| social_sciences | 3 137 | 0.551 / 0.550 | 0.586 / 0.586 | 0.594 / 0.578 | 0.559 / 0.578 | −0.035 [−0.052, −0.019] | −0.036 [−0.054, −0.019] | 278 / 389 | −0.043 [−0.060, −0.025] | −0.028 [−0.047, −0.009] |
| other | 3 172 | 0.496 / 0.480 | 0.526 / 0.526 | 0.540 / 0.529 | 0.500 / 0.525 | −0.031 [−0.048, −0.014] | −0.045 [−0.062, −0.027] | 308 / 406 | −0.045 [−0.062, −0.028] | −0.049 [−0.068, −0.029] |

Every AraRooPat − BPE-16K interval lies below zero. The five subjects with the largest |AraRooPat − BPE-16K| (char) are world_religions (humanities, n 176) −0.142 [−0.222, −0.060], formal_logic (humanities, 131) −0.137 [−0.238, −0.036], high_school_chemistry (stem, 208) −0.120 [−0.192, −0.047], college_computer_science (stem, 105) −0.114 [−0.204, −0.021] and high_school_computer_science (stem, 105) −0.105 [−0.200, −0.010]. They are thin: 105–208 rows each, intervals 0.15–0.20 wide, the five most extreme of 57 subjects. No subject-level test was pre-registered, and the category test on the same rows does not separate the categories (next table).

*3. Culture-MMLU, rotation-averaged decision, subset, per category* (char = PMI):

| category | n | AraRooPat / BPE-16K / native v5 / base | AraRooPat − BPE-16K | McNemar | AraRooPat − native v5 | same rows at rotation 0 (standard): AraRooPat − BPE-16K char / PMI |
|---|---|---|---|---|---|---|
| stem | 210 | 0.476 / 0.476 / 0.519 / 0.581 | +0.000 [−0.063, +0.065] | 24 / 24 | −0.043 [−0.105, +0.020] | +0.033 [−0.038, +0.103] / +0.005 |
| humanities | 338 | 0.420 / 0.441 / 0.444 / 0.420 | −0.021 [−0.067, +0.026] | 29 / 36 | −0.024 [−0.069, +0.022] | −0.024 / −0.030 |
| social_sciences | 227 | 0.533 / 0.568 / 0.586 / 0.604 | −0.035 [−0.094, +0.023] | 19 / 27 | −0.053 [−0.107, +0.000] | −0.040 / −0.018 |
| other | 225 | 0.471 / 0.578 / 0.564 / 0.560 | **−0.107 [−0.169, −0.042]** | 16 / 40 | −0.093 [−0.159, −0.028] | −0.080 [−0.143, −0.018] / −0.067 |

Composition, subset against full benchmark: stem 210 (0.210) vs 3 248 (0.227); humanities 338 (0.338) vs 4 770 (0.333); social_sciences 227 (0.227) vs 3 137 (0.219); other 225 (0.225) vs 3 172 (0.221). All shares agree within 1.7 points.

*4. Heterogeneity p (category / Group; pair; char, PMI).*
- Culture-MMLU, rotation-averaged: AraRooPat − BPE-16K **0.054** (observed spread 0.107 against a null 95th percentile of 0.108); AraRooPat − native 0.317.
- Culture-MMLU, standard, full: AraRooPat − BPE-16K 0.237 / 0.521; AraRooPat − native **0.003** / 0.404.
- Arabic-Exam, rotation-averaged: Group 0.490 (AraRooPat − BPE-16K) / 0.841 (− native); Level 0.905 / 0.443.
- Arabic-Exam, standard, full: Group **0.007** / 0.192 (AraRooPat − BPE-16K), **0.009 / 0.006** (− native); Level **0.042** / 0.071, **0.027 / 0.001**.

The subset's Level partition includes `Prof` with 7 rows, and its null spread is wide (95th percentile 0.41).

*5. Fragmentation terciles* (the ratio's tercile boundaries; gaps T1 / T2 / T3 from the least to the most AraRooPat-fragmented prompts):
- *Culture-MMLU, full.* AraRooPat / BPE-16K boundaries 1.594 / 1.708 (range 0.873–2.375; tercile means 1.491 / 1.651 / 1.806):
  - AraRooPat − BPE-16K char −0.033 / −0.021 / −0.025 (p 0.407), PMI −0.051 / −0.025 / −0.032 (p 0.026, not monotone);
  - AraRooPat − native char −0.057 / −0.030 / −0.048 (p 0.015, not monotone).
- *Culture-MMLU, full.* AraRooPat / native boundaries 1.244 / 1.309 (range 0.963–1.716): AraRooPat − BPE-16K char −0.024 / −0.022 / −0.033 (p 0.478), PMI −0.031 / −0.030 / −0.048 (p 0.176).
- *Culture-MMLU, subset, rotation-averaged.* Boundaries 1.600 / 1.712 and 1.247 / 1.310:
  - AraRooPat − BPE-16K by the BPE ratio: −0.021 [−0.075, +0.031] / −0.066 [−0.111, −0.021] / −0.030 [−0.080, +0.021] (p 0.396, not monotone);
  - by the native ratio: −0.018 / −0.033 / −0.066 (p 0.358, monotone);
  - AraRooPat − native: p 0.612 / 0.547.
- *Arabic-Exam.* Full boundaries 1.556 / 1.670 and 1.248 / 1.318; subset 1.582 / 1.696 and 1.264 / 1.333. Subset, rotation-averaged, AraRooPat − BPE-16K by the BPE ratio: −0.042 / +0.015 / +0.024 (p 0.177).

*6. The Arabic-Exam control.* Rotation-averaged, subset, AraRooPat − BPE-16K by Group:
- STEM (230) +0.030 [−0.037, +0.095];
- Social Science (288) −0.003 [−0.058, +0.050];
- Humanities (239) +0.017 [−0.040, +0.073];
- Language (101) −0.050 [−0.154, +0.049];
- Other (142) −0.042 [−0.111, +0.027].

Every interval includes zero and p is 0.490. Under the standard decision on the full dumps the Group partition does fire (char p 0.007): STEM −0.037 [−0.055, −0.019], Social Science −0.045 [−0.061, −0.029], Humanities −0.040 [−0.056, −0.024], **Language −0.089 [−0.120, −0.059]**, Other −0.051 [−0.068, −0.033]. A supplementary split, not pre-registered (`_letter_slot/_audit/residual_gold_slot_split.{py,json}`), shows that in every Group the standard-decision gap sits on the gold-slot-1 rows (−0.153 to −0.229; gold-slot-1 shares 0.299–0.352). On the other rows the gap is −0.034 for Language against −0.013 to +0.047 elsewhere, so under the official prompt the Arabic-language subjects are the one group where AraRooPat also trails off the أ rows. The subset's Group composition is within 3.7 points of the full benchmark's. Its Level composition is not (Primary 0.134 vs 0.230, High 0.471 vs 0.353), because the subset holds four-choice rows only.

**Readings under the pre-registered rules** (Culture-MMLU, rotation-averaged, AraRooPat − BPE-16K; char and PMI coincide):
- **R1 *concentrated*: does not fire.** The category-spread p is 0.054, above 0.05 by 0.004. The Monte-Carlo standard error of a p near 0.05 at 5 000 permutations is 0.003, so another permutation seed could land on either side. The rule is read as written. The one category whose interval excludes zero is `other` (−0.107 [−0.169, −0.042]); stem, humanities and social_sciences include zero.
- **R2 *broad*: fires.** p ≥ 0.05 and the pooled −0.039 [−0.068, −0.011] excludes zero.
- **R3 *fragmentation*: does not fire.** By the BPE ratio p is 0.396, not monotone; by the native ratio p is 0.358, monotone but not significant.
- **Control: silent** under the rotation-averaged decision (Group p 0.490, Level 0.905, terciles 0.177 / 0.583).

*Consistency.* The full dumps under the standard decision give the same gap in every category (−0.030 / −0.015 / −0.035 / −0.031 char, all below zero, p 0.237). The subset's `other` rows are already −0.080 at rotation 0 on the same 225 rows, against −0.031 on the full benchmark's 3 172. So the subset's lean towards `other` belongs to the rows drawn, not to the rotation-averaged decision.

*Supplementary, not the rules' pair:* under the standard decision AraRooPat − native v5 is heterogeneous across Culture-MMLU categories (char p 0.003; stem −0.070 against humanities −0.029).

**Reading.** The Culture-MMLU residual is a broad recognition gap of about three to four points. It is spread over the four categories: every full-dump category interval lies below zero and the category test separates nothing. It is not tied to how much more AraRooPat fragments the prompt; AraRooPat's prompts are 1.49–1.81 × BPE-16K's and 1.19–1.37 × native's in tokens across the terciles. No subject area stands out once the subset's sampling is set against the full benchmark.

For the two proposals on the table (both applied to nothing):
- *Rotation-averaged scoring as the protocol* would remove the Arabic-Exam gap to BPE-16K, which is the letter-prior artefact of §3.11–§3.12. It would leave a real, broad Culture-MMLU deficit of about 0.04.
- *The vocabulary-side letter lever* (one token per standalone choice letter) targets the أ effect, which the rotation-averaged decision already removes by construction. It is therefore not a candidate for closing this residual.

The Arabic-Exam `Language` observation under the official prompt is a separate, supplementary lead. The residual bears on recognition only. On the generation side the second judge keeps the AraRooPat − BPE-16K lead, at about 0.57 of its Gemma size (J1), and AraRooPat still cannot be separated from native v5 (J2) — see *P1*. The rater's check waits on `v5_blind`.

**Artifacts.** `outputs/experiments/qwen_native_vs_araroopat/_letter_slot/residual_by_subject.{md,json}`, `_letter_slot/_audit/residual_gold_slot_split.{py,json}`, `_letter_slot/_logs/residual_by_subject.stdout`; `araroopat_3phase_v5_distill/freeform_judge/_superseded/{gpt56_terra_2026-09-24_cells_probe.parquet,README.md}`; `_audit/judge_gpt56_terra_2026-09-26_attempt1_sandbox_no_network.log`, `_audit/judge_gpt56_terra_2026-09-26.log` (attempt 2, the first invocation), `_audit/judge_gpt56_terra_2026-09-26_run2.log` (the second invocation), `_audit/judge_2026-09-26_mtimes_before_{all_metrics,judge_files}.txt` and `_audit/judge_2026-09-26_run2_{mtimes_before_all_metrics,mtimes_before_judge_files,sha_before}.txt`; `<cell>/freeform_judge/gpt56_terra.parquet` in the five cells, their merged summaries in `all_metrics.json`, `comparison_report.{txt,json}` and `freeform_judge_report.json`; `_audit/judge_validation_2026-09-26.{py,json}` and `_audit/judge_length_2026-09-26.{py,json}`; `freeform_rating/v5_blind.json` (the set, with the hidden cells; ratings will land in `freeform_rating/v5_blind.ratings/`). Commits: `4495caa` (rating-set cells), `c1b49f1` (the residual script), `4854d4c` (this section's first documentation) and the documentation commit of the P1 / P2 completion.
...
15. **Blind human check on the distilled arms** (console Rate tab, 50 prompts × 3 variants: `teacher_aya_expanse_32b_m2_c2400`, `araroopat_3phase_v5_distill`, `native_qwen3_sft_v5_distill`, then a second set with `bpe_16k_3phase_v5_distill`): the three v5 cells write at the teacher's length, and the judge's length bias is the standing caveat of this eval — the +0.30 AraRooPat − BPE interval and the +0.24 to +0.28 distillation gains should be confirmed by a rater before they are quoted. No GPU. **Status (2026-09-26, §3.13): set built, awaiting the rater.** `v5_blind` = 50 prompts (25 where the two judges disagree most, 25 at random) × the four cells (teacher, AraRooPat v5, native v5, BPE-16K v5) = 200 items, fully crossed, in `freeform_rating/v5_blind.json`; the second judge it was drawn with is in (item 27). Rating takes two to three hours; *agreement* follows.
...
26. **Done (§3.11, 2026-09-25).** *Outcome:* the deficit follows the letter أ, not the slot (see §4 finding 9); the demonstrations and the terminal token are not the carrier; under a letter-invariant rotation-averaged decision the Arabic-Exam gap to BPE-16K vanishes and a −0.04 Culture-MMLU gap remains. *What follows:* (a) decide whether letter-scored MCQ is scored by cyclic-permutation averaging for every arm (×4 cost; the §3.11 proposal) before quoting letter-task gaps across tokenizers; (b) ~~where AraRooPat v5's preference for the letter د comes from~~ — **done, §3.12** (the Phase 1 and Phase 2 checkpoints of both from-scratch arms under the four rotations, 16 cells, 2 h 46 min GPU): the AraRooPat-specific أ deficit to BPE-16K is present after Phase 1 at its v5 size; Phase 3 created the absolute أ aversion — by a letter-prior shift it applies to every arm, larger for BPE-16K — and the د preference; the slot-1 aversion is present from Phase 1 in both from-scratch arms. What follows, as proposals for the next campaign, applied to nothing and each for every arm: letter-rotation augmentation of the Phase 3 MCQ records (the pre-registered consequence of the rule that fired — the records confound letter and slot) and one token per standalone choice letter (item 20; supported by the paired contrast, not by the pre-registered inherited rule), besides (a); (c) ~~the Culture-MMLU residual under the letter-invariant decision, by subject, from the existing dumps~~ — **done, §3.13** (`scripts/mcq_residual_by_subject.py`, CPU only). The residual is *broad* (R2): the category-spread p is 0.054, so R1 misses by 0.004 against a Monte-Carlo SE of 0.003; the pooled −0.039 [−0.068, −0.011] excludes zero; there is no fragmentation trend (R3 p 0.396 / 0.358); the Arabic-Exam control is silent (Group p 0.490). On the full dumps under the standard decision every Culture-MMLU category trails BPE-16K by −0.015 to −0.035 (char, p 0.237). The subset's larger `other` gap (−0.107) is already present at rotation 0 on the same 225 rows, so it belongs to the draw. Consequence for (a): rotation-averaged scoring would leave a real ≈ 0.04 Culture-MMLU deficit, which the one-token-per-letter lever cannot address. *The original item:* **The first-option / أ deficit under AraRooPat** (§3.10 *Few-shot handling*): the 3-shot shape is not mishandled by any cell (measured), but AraRooPat v5 scores 0.32 / 0.28 when the answer is أ against 0.49–0.62 on the other letters, and a post-hoc calibration does not move it. Letter and position are confounded on the benchmarks. Two cheap eval-only re-evals on a seeded 1 000-row subset of Arabic-Exam and Culture-MMLU for the three v5 arms would separate them: `num_fewshot: 0` (does the deficit survive without demonstrations) and a shuffled-options pass (gold moved uniformly over the four slots, letters unchanged — position); if it is the slot, look at how the first option line is tokenised after the question under AraRooPat. A third, for the mechanism: a per-token re-score of the three-token letter continuation on the same subset (log P of `[LIT_BEGIN]`, `[CHAR_x]`, `[LIT_END]` per choice — a literal beginning with أ is almost always a longer word in the corpus, so the `[LIT_END]`-after-hamza term is a candidate). Whatever scoring rule comes out of it applies to every arm as one rule, never to AraRooPat alone.
27. ~~**The second judge on the v5 cells**~~ — **done, §3.13 P1** (2026-09-26). `gpt56_terra` over the teacher ceiling, the three v5 arms and the native base: 1 250 verdicts, parse failures 0, in two invocations of one YAML on one day. The first ran out of API credits after AraRooPat v5 and BPE-16K v5; the second, without `--overwrite`, judged the remaining three. The 09-24 probe file is preserved in `_superseded/`.
    - Outcome: J1 fires (AraRooPat − BPE-16K +0.172 [+0.048, +0.300]); J2 holds (AraRooPat − native v5 −0.048 [−0.172, +0.080]); J3 QWK 0.737–0.808 per cell; J4 the gap survives on length-matched prompts under both judges.
    - What remains is the rater (item 15). The API judge's scale is compressed at the top: the teacher 3.308 against Gemma's 3.652, few 5s. So distillation gains read smaller under it. If a third judge is ever added, choose one with a different scale behaviour, not another reasoning-API model of the same kind.
...
§3.13 (2026-09-26): **none.** CPU only: the residual script took 24 s per run (two runs); the five named test files 70 passed in about 20 s (21 / 8 / 20 / 9 / 12); the full suite on CPU 1 596 passed, 2 skipped (the `bpe_32k` / `wordpiece_32k` round-trip tests: those tokenizers are not on this machine) in 394 s. API (gpt-5.6-terra, no GPU): 1 250 verdicts in all — attempt 1 made no call (no network in the sandbox); the first invocation (attempt 2) made 500 verdicts plus 68 refused `HTTP 429` retries before the credits ran out (482 s); the second invocation (Fable session) made 750 (490 s). The P1 analysis scripts run in 6 s and < 10 s on CPU; the set build is instant.
...
§3.13 artifacts: listed under §3.13 *Artifacts*. Commits: `4495caa` (`build_set(cells=…)`, the route, the Rate tab's cell chips + baseline select, `tests/test_freeform_console.py::TestRatingCells`), `c1b49f1` (`scripts/mcq_residual_by_subject.py` + `tests/test_mcq_residual_by_subject.py`), `4854d4c` (the first §3.13 documentation), and the documentation commit of the P1 / P2 completion. P1's numbers: `_audit/judge_validation_2026-09-26.json` (per cell, the nine pairs under both judges, sub-scores, strata, retained shares, reproducibility) and `_audit/judge_length_2026-09-26.json`.
...
New in §3.13 (no cells): `_letter_slot/residual_by_subject.{md,json}` and `_letter_slot/_audit/residual_gold_slot_split.{py,json}`. Kept under `araroopat_3phase_v5_distill/freeform_judge/_superseded/` with a README: `gpt56_terra_2026-09-24_cells_probe.parquet`, the 09-24 verification verdicts of the judge stage's `--cells` feature, copied before the five-cell `gpt56_terra` run, whose first invocation replaced the live file beside it at 08:04. Every cell of the v5 comparison — `teacher_aya_expanse_32b_m2_c2400`, `araroopat_3phase_v5_distill`, `bpe_16k_3phase_v5_distill`, `native_qwen3_sft_v5_distill`, `native_qwen3_base_m2_c2400` — now holds `freeform_judge/gpt56_terra.parquet` beside `gemma4_31b.parquet`. The experiment folder gains `freeform_rating/v5_blind.json`, the blind set with its hidden cells. `araroopat` (v1) and `araroopat_3phase_v2` also hold older `gpt56_terra` files, untouched.
