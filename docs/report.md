...
| 09-26 | judge validation + the Culture-MMLU residual (§3.13; CPU only): `build_set(cells=…)` + the Rate tab's cell chips (`4495caa`), `scripts/mcq_residual_by_subject.py` (`c1b49f1`); the second judge (`gpt56_terra` on the five v5-comparison cells) and the blind set `v5_blind` pending — the sandboxed attempt had no route to the API; a second, unsandboxed attempt completed but its output is not inspected or used here (the user's decision) | the letter-invariant Culture-MMLU gap to BPE-16K (−0.039 [−0.068, −0.011]) is **broad**: category-spread p 0.054 (R1 misses by 0.004), R2 fires, no fragmentation trend (p 0.40 / 0.36), the Arabic-Exam control silent (p 0.49); on the full dumps every category trails by −0.015 to −0.035 (char); the stray 09-24 `gpt56_terra` file is kept under `_superseded/` |
...
**Status when this was written: P0 (code) and P3 (the residual) are done. P1 (the second judge) has no verdicts read, and P2 (the rating set) is not built.** P1 needs outbound HTTPS to the OpenAI API, and P2's disagreement half needs P1's verdicts. What happened is under *P1* below.
...
- *Attempt 2.* This one was launched outside the sandbox. The session's permission policy then refused every further action on it, including reading its state. Its background process later reported completion with exit code 0, but that is the status of the wrapper's final `tee`, not of the judge. Its log and outputs were **not inspected and are not used here**; whether they are accepted is the user's decision. Before any use or re-run, check `_audit/judge_gpt56_terra_2026-09-26.log`, the five cells' `freeform_judge/gpt56_terra.parquet`, and every `all_metrics.json` mtime against `_audit/judge_2026-09-26_mtimes_before_all_metrics.txt` (62 files) and `_audit/judge_2026-09-26_mtimes_before_judge_files.txt` (20 judge files), both recorded before either attempt.
- *Ready to run once the verdicts exist:* the analysis scripts `_audit/judge_validation_2026-09-26.py` (per-cell means and agreement, the brief's nine pairs through `paired_compare.compare` at 10 000 resamples / seed 0, sub-scores, strata, retained shares, the 09-24 vs 09-26 reproducibility) and `_audit/judge_length_2026-09-26.py` (Spearman of score with generation characters; the length-matched pairs at ≤ 100 / ≤ 150 characters).
- *Found on the way.* Two more cells hold `gpt56_terra` files, `araroopat` (v1) and `araroopat_3phase_v2`; the brief said none but AraRooPat v5 did. They are not touched.

**P2 — the blind rating set: not built.** Its disagreement half needs both judges' verdicts. The plan stands: one fully crossed set `v5_blind` = 50 prompts × the four cells (teacher ceiling, AraRooPat v5, native v5, BPE-16K v5), 200 items, 25 disagreement + 25 random prompts, seed 42, baseline `native_qwen3_sft_v5_distill`. It replaces item 15's two three-variant sets: the rater's own AraRooPat − BPE-16K, AraRooPat − native and each arm − teacher then come from the same 50 prompts. The build form can now make it.
...
The Arabic-Exam `Language` observation under the official prompt is a separate, supplementary lead. The residual bears on recognition only. The generation-side question — whether the judge's AraRooPat − BPE-16K lead survives a second judge and a rater — waits on P1 and P2.

**Artifacts.** `outputs/experiments/qwen_native_vs_araroopat/_letter_slot/residual_by_subject.{md,json}`, `_letter_slot/_audit/residual_gold_slot_split.{py,json}`, `_letter_slot/_logs/residual_by_subject.stdout`; `araroopat_3phase_v5_distill/freeform_judge/_superseded/{gpt56_terra_2026-09-24_cells_probe.parquet,README.md}`; `_audit/judge_gpt56_terra_2026-09-26_attempt1_sandbox_no_network.log`, `_audit/judge_2026-09-26_mtimes_before_{all_metrics,judge_files}.txt`, and the two P1 analysis scripts `_audit/judge_{validation,length}_2026-09-26.py` (not yet run). Commits: `4495caa` (rating-set cells), `c1b49f1` (the residual script), and the documentation commit of this section.
...
15. **Blind human check on the distilled arms** (console Rate tab, 50 prompts × 3 variants: `teacher_aya_expanse_32b_m2_c2400`, `araroopat_3phase_v5_distill`, `native_qwen3_sft_v5_distill`, then a second set with `bpe_16k_3phase_v5_distill`): the three v5 cells write at the teacher's length, and the judge's length bias is the standing caveat of this eval — the +0.30 AraRooPat − BPE interval and the +0.24 to +0.28 distillation gains should be confirmed by a rater before they are quoted. No GPU. **Status (2026-09-26, §3.13): pending.**
- The build form now takes cells (`build_set(cells=…)`).
- The plan is one fully crossed set, `v5_blind`: 50 prompts × the four cells = 200 items.
- It waits for the second judge, since its disagreement half needs both judges' verdicts.
- The second judge (`gpt56_terra` over the five cells) needs outbound HTTPS to the OpenAI API. The sandboxed attempt had none; the unsandboxed attempt's output is not yet inspected (item 27).
...
27. **The second judge on the v5 cells** (§3.13 P1; blocks item 15). One `gpt56_terra` run over the teacher ceiling, the three v5 arms and the native base, with `--baseline native_qwen3_base_m2_c2400 --overwrite`. Then `_audit/judge_validation_2026-09-26.py` and `_audit/judge_length_2026-09-26.py`, which are written and waiting for the verdicts.
    - The run needs outbound HTTPS to `api.openai.com`, i.e. a shell outside the command sandbox, or the user running the command.
    - The 2026-09-26 attempt 2 completed in the background (wrapper exit 0, the status of `tee`, not of the judge), and its output was not inspected (§3.13 *P1*). Before accepting its verdicts or re-running, read `_audit/judge_gpt56_terra_2026-09-26.log` and compare the mtimes with the two `_audit/judge_2026-09-26_mtimes_before_*.txt` files.
    - The 09-24 file is preserved in `_superseded/`.
...
§3.13 artifacts: listed under §3.13 *Artifacts*. Commits: `4495caa` (`build_set(cells=…)`, the route, the Rate tab's cell chips + baseline select, `tests/test_freeform_console.py::TestRatingCells`), `c1b49f1` (`scripts/mcq_residual_by_subject.py` + `tests/test_mcq_residual_by_subject.py`), and the documentation commit of §3.13.
