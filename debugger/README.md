...
| **Form** | the *resolved* config (the file merged over `base.yaml`, exactly what `load_config` produces), rendered from the Pydantic JSON schema. Hand-built widgets for the parts that matter: sweep tokenizer cells (type from the registry, `vocab_sizes`, **typed params** from the tokenizer's `param_spec()` — the same rows as the task cards, greyed *code* defaults, pin, raw toggle, undeclared-keys group — plus *fill from `configs/tokenizers/<type>.yaml`*, which writes the preset's recommended values as explicit keys; reorder / remove), the eval-task checklist with a **typed card per task** (see the next row), the three phase cards (enable toggle in the header, dataset checklist, mix-mode hint showing `steps = mix_tokens / (batch_size × block_size)`, a `mixture` sub-card for the ratio-controlled Phase 3 with its own hint `steps = total_examples / batch_size`, and the mixture summarised in the card badge), `corpus_params` (per-corpus loader parameters such as `aya_ar.include_datasets`), model presets from `configs/models/`. The `output_dir` row carries a **mode selector**: *infer from name* locks the field to `outputs/experiments/<name>` and follows every rename (also after *YAML → Form* or an assistant edit); *cell of experiment…* locks it to `outputs/experiments/<experiment>/<name>` with the experiment folder picked from the existing ones (`GET /api/experiments`: the folders under `outputs/experiments/` plus the parents named by saved configs, `_superseded` skipped) — the layout the Free-form tab, the judge and `compare_results.py` read as one experiment; *custom path* takes anything. The mode is re-derived from the config on load, so the conventional configs open in *infer* and the campaign cells (`outputs/experiments/qwen_native_vs_araroopat/<cell>`) open in *cell* with their experiment preselected — renaming those keeps them under it. The Clone and Re-eval dialogs use the same widget. Params boxes take flat `key: value` YAML. Every field label carries a `?`: hovering it shows what the field means and a concrete example (the `HINTS` table at the top of the page script, keyed by config path with `*` for phase names and list indexes). |
...
| **⟳ Re-eval…** (toolbar, or the `⟳` on a list row that has results) | builds an **eval-only config from a finished cell's checkpoint**: the dialog lists the cells under the config's `output_dir` that hold a `training/<phase>` checkpoint (discovered from disk like the Results tab; `_superseded/` skipped; a single run is its own cell), the checkpoints of the chosen cell (last phase first), the tasks (the source's ticked, params copied from it), the experiment name (`<cell>_reeval`), the `output_dir` (mode widget, default *cell of experiment* = a sibling cell folder of the source cell — `outputs/experiments/qwen_native_vs_araroopat/native_qwen3_sft_reeval`), a prefilled description. `POST /api/configs/reeval` (`reeval_config`) returns the source config with `model.name_or_path` = the checkpoint directory, every phase `enabled: false`, `sweep.tasks` = the ticked tasks, the tokenizer block of the cell's `config.json` (`load_path` = its saved tokenizer for a from-scratch type so nothing is retrained; `null` for `native_*`), fresh provenance — **nothing is saved**: it opens as an unsaved config in the form (Save… / ▶ Start as usual). The hint warns when the cell's tokenizer is CharBERT / Farasa-CharBERT / char-JABER / Charformer (`from_pretrained` on the checkpoint rebuilds the vanilla architecture and leaves their custom embedding modules random — the caveat of `scripts/dump_eval_rows.py`), and when a ticked task has no `eval_rows` dump in the cell. |
| **⇄ Compare…** | pick another saved config: a table of every *resolved* path whose value differs (each side merged over `base.yaml`), grouped by section; lists of scalars compare whole, lists of mappings of equal length element by element (`sweep.tasks.0.params.max_length`), `absent` when a key exists on one side only, `created_at` / `runs` ignored. `←` copies the other value into the form (an `absent` copy deletes the key) and re-diffs. `POST /api/configs/diff` (`diff_configs`). |
| **▶ Start run…** | validates, then shows what will run (source, cells, tasks, phases, cells that will be skipped) plus **every task's resolved params** (spec defaults overlaid by the file's; `●` = written in the file, set or pinned; `num_fewshot` shows the injected `evaluation.num_fewshot` when the file does not set it) so the budget / markers / few-shot a run will use are visible before launch; when `output_dir/all_metrics.json` exists (a single run) a red *results exist — the run will overwrite them* line requires a tick before Start is enabled (a sweep still skips its finished cells), and a single run whose `output_dir` already holds cell folders with results is warned about. Then the CLI overrides: `--sweep` (auto = more than one tokenizer cell, as `run_experiment.py` requires), `--device`, `--seed`. One run at a time by default — two full-FT jobs do not fit the H100; *run concurrently* overrides. An unsaved form is started from a snapshot, no file needed. |
...
**Compare mode (2026-09-24).** The *mode* selector switches the tab from one cell's rows to **several cells' answers to the same prompts, side by side** (`GET /api/freeform/compare`): tick 2–6 cells (the default pair is the viewed cell + the `native_*` cell whose name shares the most trailing parts — `araroopat_3phase_v5_distill` pairs with `native_qwen3_sft_v5_distill`, not with `native_qwen3`), pick the **anchor** every Δ is measured against, and the table shows one row per prompt with one column per cell (clipped generation + judge score / flags / chrF / chars / stop reason, and **the judge's own comment for that generation** clipped to two lines — hover for the rest; the whole verdict travels with the row, so every column reads alike). Clicking a row opens the full answers as panes, each with that cell's verdict, flags and rationale — no column gets more detail than another. Only the prompts **every** selected cell answers are shown; the rest are counted per cell in the summary strip rather than dropped silently. The ordinary filters apply under *filters match*: `any cell` (default) / `all cells` / `the anchor` — "where did any variant loop" and "where did they all loop" are different questions. The cross-cell sorts are *cells disagree most (spread = max − min over the selected cells)*, *Δ vs the anchor* both ways, *chrF Δ* and *length ratio*. The summary strip becomes one card per cell (judge mean, chrF, mean chars, loops, EOS **over the shown prompts**) with its Δ against the anchor — descriptive only: the bootstrap CI and win/tie/loss live in the judge stage's `vs_baseline` and in `scripts/judge/paired_compare.py`. *export CSV* writes the shown prompts with one column group per cell (generation, chrF, judge score, judge rationale, stop reason, characters). Single-cell mode is unchanged.
