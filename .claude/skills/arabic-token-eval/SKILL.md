...
The same signature-gating pattern is reused at *training* time for `training.completion_only_loss`: the kwarg lives on `LightEvalBenchmarkTask.get_dataloader()`, and `pipeline/experiment.py` checks `inspect.signature(task.get_dataloader).parameters` before passing it. QA / text_generation log a warning and ignore — same shape as `failure_report_dir`. When you add a future training-time switch (e.g. token weighting, focal loss, label smoothing per-task), reach for this pattern rather than widening `BaseTask`.

A second related switch — `training.ft.enabled` (default `True`) — is the master on/off for the fine-tuning step. The pipeline guards both dataloader construction (step 5) and the trainer call (step 6) on this flag. When `false`, evaluation runs on the unmodified pretrained model. This is the path used by the `native_llama_no_sft_benchmark_sweep` baseline (see "native_llama as a baseline-only wrapper" below). The flag lives on `TrainingConfig.ft.enabled` rather than `TrainingConfig.enabled` so future fine-tuning sub-flags (`ft.freeze_layers`, `lora.*`, etc.) have a stable namespace.

### `native_llama` as a baseline-only wrapper

The `native_llama` tokenizer ([tokenizers/native_llama.py](src/arabic_eval/tokenizers/native_llama.py)) wraps Llama-3.2-1B's pretrained tokenizer. It is **not** part of the 8-tokenizer comparison sweeps — it lives in a different vocab regime (128256 vs 16/32/50K), uses a different special-token convention (pad=eos=128001, bos=128000, unk=128002 = `<|reserved_special_token_0|>`; IDs 0–3 are regular ASCII in Llama's vocab so the project's standard (0,1,2,3) convention would have collided), and `train()` is short-circuited.

Use it for two reference baselines: (a) `native_llama_no_sft_benchmark_sweep.yaml` with `training.ft.enabled: false` measures the pretrained-model ceiling; (b) `native_llama_with_sft_benchmark_sweep.yaml` runs the same SFT pipeline with the native tokenizer to isolate the cost of the from-scratch tokenizer choice. Confirmed empirically: pretrained-no-SFT on ACVA scored 0.6078, **above** bpe_50k-with-SFT (0.5958) — useful evidence that ACVA's discriminative ceiling is label-noise, not the SFT setup.

The pad=eos=128001 collision is harmless because the collators build `attention_mask` from "real token positions wrote a 1, else 0" — they never compare against `pad_id`, so `labels[attention_mask == 0] = -100` correctly masks trailing pad-EOS while real EOS in mid-sequence stays attention_mask=1 and contributes to loss. Don't try to "fix" this by adding a dedicated pad token — it's the standard HF pattern for causal LMs without one.

### `completion_only_loss`: prompt masking via longest-common-prefix, not `len(prompt)`

The opt-in `training.completion_only_loss` flag masks prompt tokens to `-100` so only the answer span contributes to loss. It is NOT implemented as `labels[:len(prompt_enc)] = -100` — that's a trap. Many tokenizers auto-append `</s>` to standalone encodings: `prompt_enc = [BOS, ..., EOS]` length P, `full_enc = [BOS, ..., answer, EOS]` length F > P. So `prompt_enc[P-1] = EOS` but `full_enc[P-1] = first answer token` — naive length-based masking eats the first answer token.

The correct boundary is the longest-common-prefix between `prompt_enc.input_ids` and `full_enc.input_ids`. See `get_dataloader` in [tasks/lighteval/base.py](src/arabic_eval/tasks/lighteval/base.py). When LCP equals `len(full_enc)` (prompt got truncated to the same length as full), the example is dropped and counted in a one-time INFO log alongside kept-count and avg completion-tokens-per-example. If you ever add another tokenizer family that auto-appends terminators (or one that prepends them differently), the LCP approach handles it without changes — don't switch to length-based masking.

**Empirical caveat: this flag is NOT a fix for ACVA-shape always-class collapse.** A 2x2 ablation on `bpe_50k` + ACVA across `train_split_ratio ∈ {0.10, 0.50}` × `completion_only_loss ∈ {F, T}` showed mask alone compresses margins without breaking the trap, and mask + more data REVERTS to always-class. The flag is genuinely opt-in for clean tasks where the answer-token signal is the binding constraint; it is *negative* for noisy short-answer 2-class MCQ. See "Things to push back on" for the bullet to cite when this comes up.
...
- `_format_eval_context` / `_build_continuations` / `_format_sft_text` are overridden in `AlghafaTask`, each branching on `_is_word_scored(ex)`. Word-scored: drop the choice listing, score answer text directly. Letter-scored: inherit the base implementation.
...
Both bugs are fixed; the regression tests in `tests/test_alghafa_parser.py` pin them down. **The lesson is that the test fixture (`SAMPLE_ALGHAFA_DATA` in `tests/test_e2e_sweep.py`) was constructed with labels picked to match the buggy parser** rather than the dataset's actual convention — so the test passed against the bug. Cross-check parsers against the dataset's real schema (and against LightEval's reference adapter when one exists — `lighteval/tasks/multilingual/adapters.py:alghafa_adapter` would have caught both bugs at PR time). See `feedback_verify_dataset_schema_on_real_rows.md` in memory.
...
- **"Set `training.completion_only_loss=true` to fix the always-class collapse on ACVA / similar noisy short-answer MCQ."** No — empirical 2x2 ablation on `bpe_50k` + ACVA crossed `train_split_ratio ∈ {0.10, 0.50}` with `completion_only_loss ∈ {F, T}`: mask alone compresses margins (98.1% < 1e-3) without breaking the always-صح trap; mask + more data REVERTS to always-class (99.8% صح + 99.8% near-tie); the only cell that broke the trap was `(split=0.50, mask=F)`, and it landed *below* majority-class. The flag is plumbed and works correctly; it's just *negative* on this task family. Artifacts: `outputs/experiments/ACVA_{top_tokenizers_benchmark_sweep, completion_loss_ablation, split_ratio_ablation, split_and_completion_ablation}/`. **Important correction (2026-05-03):** the from-scratch-tokenizer ACVA ceiling near 0.595 is *not* due to label noise as previously concluded — `native_llama_with_sft_benchmark_sweep` reached ACVA=0.7133 with the same SFT pipeline using the pretrained tokenizer. The binding constraint is the from-scratch tokenizer's randomly-aligned pretrained rows, not the data. Label noise is real but not the ceiling. The actual fix for the ~0.6 cap is using a tokenizer with usable pretrained embeddings — see `native_llama_{no,with}_sft_benchmark_sweep`.

- **"Add LoRA / PEFT to speed up the SFT loop."** Today's loop is full-model — `LlamaAdapter.get_trainable_parameters()` returns every parameter; AdamW updates all of them. Adding LoRA isn't a flag, it's a new model adapter that wraps the base model with LoRA layers (and a different `get_trainable_parameters()` returning just the adapter weights). Confirm with the user that they want a new adapter (and accept the comparison-cleanliness trade-off — a LoRA-tuned variant is no longer apples-to-apples vs the existing full-FT sweeps) before starting.

- **"Add `native_llama` to the 8-tokenizer comparison table for the next sweep."** No — `native_llama` is a baseline-only wrapper, not a competitor in the comparison. It sits in a different vocab regime (128K vs 16K/32K/50K), uses a different special-token convention (pad=eos=128001, IDs 0–3 are regular ASCII in Llama's vocab so the (0,1,2,3) convention would have collided), `train()` is short-circuited, and its intrinsic/MEI scores aren't peer-comparable to the from-scratch tokenizers' numbers. Use it only via the dedicated `native_llama_{no,with}_sft_benchmark_sweep` configs to anchor the from-scratch sweeps against the pretrained ceiling.

- **"Reinit the embedding/lm_head when swapping tokenizers — that's what the doc says happens."** Read [models/embeddings/standard.py](src/arabic_eval/models/embeddings/standard.py): `resize_token_embeddings(new)` is a no-op when `new == old`, and HF's underlying resize keeps the **first N pretrained Llama rows** when `new < old`. Only newly-added rows when expanding (`new > old`) are reinitialized. So every from-scratch tokenizer in our sweeps (vocab ≤ 128256) is silently mapped onto the first N pretrained rows; SFT drifts those associations to be useful. This is *the* property that makes the `native_llama_with_sft_benchmark_sweep` baseline a meaningful control — both keep the first N pretrained rows; only the tokenizer differs. The CLAUDE.md "reinitialize those new weights" line was misleading; corrected as of 2026-05-03.
...
- **"The Alghafa results are surprisingly low — must be the from-scratch tokenizer / random row mapping / [insert model explanation here]."** Before reaching for a model-side explanation on Alghafa, **check the parser**. The 2026-05-03 fix uncovered two parser bugs (label off-by-one + sol5 truncation) that silently dropped 36 % of rows and shifted gold answers by −1 — accuracy looked depressed for reasons completely unrelated to the model. The "+31.19 pp on alghafa" finding in earlier docs was an artifact of that bug. When numbers from a benchmark "look surprisingly low" for a strong baseline (e.g. native pretrained Llama barely above random), inspect the parser against actual dataset rows from each sub-config first — it's the cheapest investigation step and rules out the most insidious failure mode. Cross-check against LightEval's reference adapter (`lighteval/tasks/multilingual/adapters.py:alghafa_adapter` etc.) when one exists for the dataset family.

- **"culture_arabic_mmlu accuracy is sub-30% — must be the from-scratch tokenizer / a small SFT split / a noisy benchmark."** Before reaching for any of those, check whether **`evaluation.score_normalization`** is set to `"pmi"` or `"char+pmi"`. Default char-norm is dominated by Llama's per-letter unigram prior on translated MMLU (the conditional log P( letter | empty MCQ context ) spans ~1.7 nats across ج/د/ب/أ — far larger than the question-conditioned signal a 1B model gets from a translated MMLU prompt). On the no-SFT failure CSV for `culture_arabic_mmlu` the predicted-letter distribution among 9,723 wrong rows was ج 45 % / أ 36 % / ب 16 % / د 3 % — yet "د" is the most common gold answer (~27 %), producing below-random accuracy. The fix is `LogProbPMINorm` (subtract the unconditioned per-continuation ll), wired up as `score_normalization: "char+pmi"`. Do NOT propose tweaking the SFT setup, the split ratio, or the tokenizer choice as the fix for sub-30% letter-MCQ accuracy without first confirming PMI is enabled — the binding constraint is the prior bias, not the SFT or the tokenizer.

- **"Just write the test fixture to match what `_parse_example` does so the tests pass."** No — that's how the Alghafa bug stayed live for months. Synthetic test fixtures must be authored against the dataset's true convention (and ideally cross-checked against LightEval's adapter), then the parser is implemented to match the fixture. `tests/test_alghafa_parser.py` shows the correct pattern; `SAMPLE_ALGHAFA_DATA` in `tests/test_e2e_sweep.py` was the cautionary tale (its labels were originally picked to match the buggy parser, with a `# 1-indexed string` comment that was pure fiction).

- **"Switch all benchmarks to word-scoring everywhere — letter-scoring is broken."** No — letter-scoring is fine for genuinely 4/5-way MCQ where each letter is rare in training and the prior matters less. The pathology is specifically on **2-way and 3-way** tasks where the unigram letter prior dominates the decision (the ACVA observation, replicated for Alghafa's binary/sentiment sub-configs). The dispatch should be per-sub-config (`AlghafaTask.WORD_SCORED_CONFIGS` is the single source of truth), not blanket. Alghafa today is the only task spanning multiple shapes; if a future benchmark adds another binary/sentiment sub-config, add it to that frozenset and don't touch the rest.
