...
`datasets: ["pretraining_mix"]` + `mix_tokens` (sole entry, `full_sequence`, `max_length == block_size`, `mix_tokens == steps × batch_size × block_size` with `steps` derivable) switches a phase to the packed raw-text mix (70 % FineWeb-2 / 20 % Wikipedia / 10 % ArabicWeb24 by default). Two stages: a tokenizer-independent **pool** (streamed, quality-filtered, dialect-gated, cross-source deduped, cached by fingerprint of the Stage A config) and a **pack** of the *whole* pool once per tokenizer (shared across experiments under `<pool>/packed/<tokenizer_fp>/`, keyed on a tokenizer content hash) at the largest budget where the token shares hold exactly. Phases draw consecutive, disjoint slices sized by `mix_tokens`; a pool too small for Σ `mix_tokens` is an error, never a wrap. Things to keep straight:

- Don't reintroduce a "budget" knob or wrap-around: the reserved tokens must equal the trained tokens (the validator enforces it), and slice stability across experiments depends on packing the whole pool with a fixed block order.
- `qa_blend: {datasets, share, split, seed}` on a mix phase swaps `round(share × blocks)` of *that phase's* blocks for SFT-format QA text (TyDiQA + ARCD train, rendered by `_format_qa_full` — the Phase 3 / eval surface form — packed EOS-separated like the pool, full-sequence loss, own cache under `cache_dir/qa_blend/<tokenizer_fp>/`, interleaved through a seeded permutation so every batch is a mixture). The share comes *out of* `mix_tokens`, not on top: the with/without arms spend identical compute and Phase 1's slice is untouched. Never blend the held-out evaluation splits or the early-stop slice (`validation` and `dev` of tydiqa/arcd are refused by the validator), and don't add answer-only masking to packed blocks — "plain text" is the point.
...
### The official TyDiQA-AR / ARCD evaluation splits are held out — and decontaminated (2026-09-17)

`configs/contamination/heldout_sets.yaml` declares them (plus a *pending* placeholder for the user's free-form eval prompt file: `kind: file, path: null` — set the path when the file exists, nothing else changes). `src/arabic_eval/data/contamination.py` matches every training passage against them (one normalizer, exact passage / paragraph / question hashes, word 8-grams — 6 for prompts — with per-pair coverage and longest run; `contaminated` = exact or coverage ≥ 0.5 or run ≥ 20 words). Things to keep straight:

- Both datasets sample Arabic Wikipedia, so the *same article intros* appear in TyDiQA train, ARCD train and ARCD test as overlapping windows. The first scan excluded 356 TyDiQA-AR train rows and 122 ARCD rows (18 % of ARCD train); those are real 20–74-word verbatim runs. Loosen `tiers.run_words` in the YAML if ARCD gets too thin, then rerun the scan.
- `scripts/check_contamination.py --write-exclusions` regenerates `configs/contamination/exclusions.json` (committed). `training.contamination_exclusions` applies it to `train` / `dev` of every corpus (phases, mixture pools, early-stop split, qa_blend); `null` disables. Never to `validation`.
- The pool filters by *content* at Stage A (`pretraining_mix.heldout_filter`), not by id, and its fingerprint includes the held-out identity: a new prompt file or a re-pinned dataset revision rebuilds the pool by itself. Tests that build pools with `mix_cfg` disable the filter; inject a `HeldoutIndex` to test it.
...
- **The budget is in characters, not tokens.** `max_output_chars` (2 400 since 2026-09-21; 1 200 capped 12 of the control's 250 answers and cut 11 finished ones) becomes a per-tokenizer `max_new_tokens` from measured chars/token (1 209 for native Qwen3, ~2 800 for char-JABER — still under the 4 096 ceiling); the same token count would give char-JABER a quarter of BPE's text. It is read from `sweep.tasks[].params` of the experiment YAML *only*: `configs/tasks/*.yaml` are console presets that never reach a run (a test pins `freeform_cidar.yaml` to the code defaults), and the cell records the budget it decoded under as `max_output_chars` next to `token_cap`. Keep decoding pure greedy: a repetition penalty or n-gram block is granularity-dependent and a confound — loops are for `degenerate_rate` and the judge.
- **Strip the trailing EOS of the prompt encoding** (every from-scratch tokenizer appends one); the marker stop is checked on decoded text, so it works for every tokenizer. **Markers are the templates' exact block labels** (`\nالسؤال:` …, `\n### الإجابة:` …, `\nفيما يلي تعليمات` / `\nفيما يلي نص`), derived from the `finetune_corpora` constants in `DEFAULT_STOP_MARKERS` — never a bare `\n###` or `\nفيما يلي`: under the v2 template `### ` is also ordinary markdown, and the bare marker cut the model's own sub-headers (5 of the 5 marker stops of the untrained control, 2026-09-21).
- **The loop stop is a *periodic tail*, not "a 4-gram repeated somewhere".** `metrics.detect_loop` fires when the decoded text *ends* in `loop_min_copies(p)` contiguous copies of one unit of `p ≤ 60` words (5 / 4 / 3 copies for p = 1 / ≤ 3 / ≥ 4; a trailing partial copy counts once the full copies are one short and it covers half the unit; the last word only has to be a prefix of its counterpart one period back because the 16-step check lands mid-word) or a *letter or digit* runs 20 times. The cut is the end of the **first** copy — everything before the loop is kept. The 2026-09-18 rule ("a word 4-gram occurs 3 times anywhere", cut before its second occurrence) cut 56 of its 68 stops on the untrained control at numbered-list headings, markdown tables and question restatements, hundreds of characters before any loop; don't reintroduce a global n-gram count, and don't let the char rule count whitespace or punctuation (table padding, `|-----|` separator rows, horizontal rules, tatweel stretches — `cidar-4688` hit both). `is_degenerate` = the stop rule ∪ the density rules (dup 4-grams ≥ 50 %, dup char 8-grams ≥ 60 %); the invariant *stop ⇒ degenerate* is fuzz-tested in `tests/test_loop_detector.py`, the real false positives (`cidar-10013`, `cidar-4688`, …) and the true loop (`cidar-3388`) are pinned against the archived dumps.
...
- **The free-form eval prompt is `_format_qa_prompt` on an `instruction` record** — it follows the training string automatically; `tests/test_freeform_task.py` pins eval prompt == training prompt. `DEFAULT_STOP_MARKERS` is built from these constants — `"\n" + label` for the five section labels and `"\n"` + the first three words of each header (`"\nفيما يلي تعليمات"` / `"\nفيما يلي نص"`, a header restart) — so the markers follow a template change; a bare `"\n###"` is deliberately not one.
...
6. `param_spec()` (classmethod, `list[ParamSpec]` from `arabic_eval.params_spec`, defaults read from the constants the constructor / `train()` use) and `configs/tokenizers/<name>.yaml` with the recommended values. The console renders the spec as typed rows and warns on undeclared keys; the tokenizer YAML is a preset the form copies (not generated), pinned by `tests/test_tokenizer_param_specs.py` as declared + typed.
...
- **"The loop stop should fire as soon as any phrase repeats three times — lists that repeat a heading are degenerate anyway."** Measured otherwise (2026-09-20): 49 of the 68 loop stops of the untrained control had a duplicate-4-gram share under 25 % — three-point answers, timelines, إعراب — and were cut at 55–61 % of their length, which moved `mean_gen_chars`, chrF and `eos_rate`. A loop is a periodic *tail*; a repeated heading is structure. Keep the stop tail-based and leave whole-text repetition to `degenerate_rate`'s density rules and the judge.
