...
Covers:
  ...
  * ``LightEvalModelWrapper.evaluate_mcq`` under ``"char+pmi"``:
      ...
      - ``accuracy`` aliases ``accuracy_pmi`` (PMI is primary — char-norm is
        dominated by class-collapse on weak-signal MCQ);
...
def test_evaluate_mcq_char_plus_pmi_emits_both_accuracies(mock_ll, tmp_path):
    ...
    # Under "char+pmi", `accuracy` aliases `accuracy_pmi` (PMI is the unbiased
    # primary metric; char-norm is dominated by class-collapse on weak-signal MCQ).
