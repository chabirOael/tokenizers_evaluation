...
- Vocab layout (deterministic ID order): specials → `[LIT_BEGIN]` / `[LIT_END]` → `[CLITICP_*]` (proclitics) → `[CLITICE_*]` (enclitics) → `[PREP_*]` (closed-class prepositions, fixed order) → `[FUNC_*]` (closed-class function words, fixed order) → `[CHAR_*]` → `[DIGIT_*]` → `[PUNCT_*]` → `[ROOT_*]` → `[PAT_*]`.
