...
- Vocab layout (deterministic ID order): specials → `[LIT_BEGIN]` / `[LIT_END]` → `[CLITICP_*]` (proclitics) → `[CLITICE_*]` (enclitics) → `[PREP_*]` (closed-class prepositions, fixed order) → `[CHAR_*]` → `[DIGIT_*]` → `[PUNCT_*]` → `[ROOT_*]` → `[PAT_*]`.
